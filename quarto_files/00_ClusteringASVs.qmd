...
```{r}
eval_clusters_full %>% select(nclust,alpha,method,ratio_avg_within_between,ratio_avg_max_min) %>% 
...
cv_clustering_df %>%
  mutate(alpha=factor(alpha0,ordered = T,labels = c('0.00','0.10','0.25','0.50'))) %>% 
  ...
  ungroup() %>% 
  ggplot(aes(x = nclust, y = avg,group=alpha,color=alpha,fill=alpha),alpha=0.1) +
...
hclus_0.50 <- stable_obj_alpha0.50$hclust_#[1:250,1:250]
