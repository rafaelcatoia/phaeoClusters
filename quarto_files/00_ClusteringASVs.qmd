...
if(running){
  ...
  list_with_clusters_alpha0 <- clusterize_ward_pam(
    distMatrix = inducedDist$normalizedAitDist,
    maxnclust = 50,
    dFrameAsv = dframe$ASV_composition$name)
  
  alpha_=0.1
  
  distMatrix_ = alpha_*inducedDist$normalizedMyDist+(1-alpha_)*inducedDist$normalizedAitDist
...
if (running){
  set.seed(1234)
alpha0_bootstrap = finding_optimal_k(
  dframe_asv = dframe$dframe %>% select(SampleKey,ASV_name,Raw.Sequence.Counts),
  B = 10,split_pct = 0.5,maxnclust_ = 30)

alpha0_bootstrap_df <- data.table::rbindlist(alpha0_bootstrap) %>% data.frame()
  saveRDS(alpha0_bootstrap,file = paste0(savingdir,'/','alpha0_bootstrap_df'))
}

alpha0_bootstrap_df = readRDS(file = paste0(savingdir,'/','alpha0_bootstrap_df'))
```
...
df_results %>% 
  ...
  filter(nclust > 1) %>% 
  select(-iteration) %>% 
  mutate(nclust=factor(nclust,levels=1:10)) %>% 
